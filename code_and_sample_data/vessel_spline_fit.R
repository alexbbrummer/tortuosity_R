vessel_spline_fit <- function(vessel, number_samples = 10000, smoothing = NULL, plot = TRUE){
  # This function performs a spline smoothing fit of the provided voxels for a given vessel.  The vessel parameter needs to be an N X 3 matrix of the (x, y, z) coordinates of the vessel voxels.  The number_samples parameter determines how many new points will be formed for defining the output vessel.  Currently we just define it as a single value, but in the future this should be modified to be representative of samples per unit arclength to maintain a constant number of samples through curves.  The smoothing parameter is a place holder for now that will be used to toggle the extent of smoothing.  This can be more complicated than just a single number.  See ?smooth.spline for more information on the use of the smoothing parameter, which in the documentation is referred to as "spar".
  library("pracma")
  ...
  # filtering will happen were.  A default smoothing parameter value is estimated, however we can control that by varying the value of spar and removing the use of cv.  Note that cv is a leave-one-out cross-validation procedure for fitting.  Annoyingly, the use of cv = TRUE yields the best fits to known curves (common functions) even with noise, but it appears to grossly overfit real vessel data.
